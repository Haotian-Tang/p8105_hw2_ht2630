...
There are `r nrow(amyloid_candidate)` rows and `r ncol(amyloid_candidate)` columns. 

Variables are `r colnames(amyloid_df)`.

And we remove patients who have NA at any year. 

Since the rows are not equal(97 in baseline dataset and 430 in amyloid dataset), there are patients who are only in one dataset. 
